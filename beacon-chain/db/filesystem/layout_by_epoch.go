...
func (l *periodicEpochLayout) pruneBefore(before primitives.Epoch) (*pruneSummary, error) {
	...
	for periodDir := range periodsToCheck {
		if err := l.fs.Remove(periodDir); err != nil {
			//This is expected if the period directory is not empty, so just debug log
			log.WithField("dir", periodDir).WithError(err).Debug("Period directory not empty or failed to remove")
		}
	}
